...
function QuestItem({ quest }: { quest: Quest }) {
  ...
  return (
    <AccordionItem className="border-b-2 border-foreground dark:border-ring" value={quest.id}>
          ...
          <div className="flex-1 text-left">
            <div className="flex flex-col sm:flex-row items-center justify-between sm:pr-10 gap-3">
              <div className="flex flex-col gap-1">
                <h3 className="text-sm font-medium text-center sm:text-left">{quest.title}</h3>
                <p className="text-xs text-muted-foreground mt-1">{shortDescription}</p>
              </div>

              <Badge variant={getStatusBadgeVariant(quest.status)} className="text-[9px]">
...
export function QuestLog({
  ...
  const sortedQuests = [...activeQuests, ...quests.filter((quest) => quest.status !== "active")];
    ...
    <Card className={cn("w-full", className)}>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base justify-between">
          Quest Log
          {activeQuests.length > 0 && <Badge className="ml-2">{activeQuests.length} Active</Badge>}
        </CardTitle>
      </CardHeader>

...
export interface CharacterSheetProps
  extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof characterSheetVariants> {
...
export function CharacterSheet({
  ...
  return (
    <Card className={cn("w-full", font !== "normal" && "retro", className)} {...props}>
      {/* Character Header */}
        ...
        <div className="flex sm:flex-row flex-col items-start gap-4">
          ...
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex flex-wrap flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              ...
              {showLevel && <Badge className="text-xs w-fit">LV. {characterLevel}</Badge>}
            </div>

            {(characterClass || characterTitle) && (
              <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
                {characterClass && <span>{characterClass}</span>}
                {characterTitle && <span className="text-amber-500">{characterTitle}</span>}
              </div>
            )}
          </div>
        </div>
      ...
      <CardContent className="space-y-6">
        ...
        {showSecondaryStats && secondaryStats && secondaryStats.length > 0 && (
          <>
            <Separator />
            <div className="space-y-3">
              <h3 className="text-sm font-bold uppercase tracking-wide">Stats</h3>
              <div className="grid sm:grid-cols-2 gap-2">
                {secondaryStats.map((stat) => (
                  <div key={stat.name} className="flex items-center justify-between py-1.5 px-2">
                    <span className="text-xs text-muted-foreground flex items-center gap-1">
                      {stat.icon}
                      {stat.name}
                    </span>
                    <span className={cn("text-sm font-bold", stat.color || "text-foreground")}>
                      {stat.value}
                      {stat.isPercentage && "%"}
        ...
        {showEquipment && equipment && equipment.length > 0 && (
          <>
            <Separator />
            <div className="space-y-3">
              <h3 className="text-sm font-bold uppercase tracking-wide">Equipment</h3>
              <div className="space-y-2">
                {equipment.map((item) => (
                  ...
                  >
                    <span className="text-xs text-muted-foreground uppercase">{item.slot}</span>
...
export function CharacterSheetStatRow({
  ...
  return (
    <div className={cn("flex items-center justify-between py-1.5 px-2", className)}>
      <span className="text-xs text-muted-foreground flex items-center gap-1">
        {icon}
        {label}
      </span>
...
export function CharacterSheetAttributeBox({
  ...
  return (
    <div className={cn("flex items-center justify-between p-2 bg-muted/30 border-2", className)}>
      <span className={cn("text-xs font-bold", color || "text-foreground")}>{shortName}</span>
